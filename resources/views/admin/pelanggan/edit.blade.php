    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.pelanggan.update', $pelanggan->id) }}" class="p-8 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="no_telepon" id="no_telepon" value="{{ old('no_telepon', $pelanggan->no_telepon) }}" placeholder="Contoh: 08123456789" class="w-full h-14 px-4 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:ring-2 focus:ring-[#1e3a5f] focus:border-transparent focus:bg-white transition-all duration-200 placeholder:text-gray-400">
                @error('no_telepon')
                    <p class="mt-2 text-sm text-red-600 flex items-center gap-1">
                        <span class="material-symbols-outlined text-sm">error</span>
                        {{ $message }}
